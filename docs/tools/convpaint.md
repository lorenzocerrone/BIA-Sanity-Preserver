# Convpaint

Convpaint is a napari plugin for interactive image segmentation using convolutional neural networks. It allows users to perform semantic segmentation with sparse annotations. Learn more about Convpaint at [Convpaint repository]()

## Environment

Convpaint is available in the `convpaint` environment.

To activate the environment, pre-fix your commands with:
```bash
pixi run -e convpaint YOUR_COMMAND
```

## Available Tasks

### Start Convpaint As Napari Plugin
Launch the Convpaint graphical user interface:
```bash
pixi run convpaint
```